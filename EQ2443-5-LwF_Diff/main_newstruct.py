...
###########################################################################################
def param_tuning_for_LS(X_train, Y_train, X_test, Y_test, LwF_flag = None, Wls_prev = None, epsilon_o = None, mu = None, eps_2 = None, eps_1=None):
    ...
    if LwF_flag == False:
        ...
        lambda_o_vec = np.geomspace(1e-10, 1e10, 11) 
        mu = 1e3
        test_acc_vec_1 = []
        test_acc_vec_2 = []
        test_acc_vec = []
        # Sweeping over a list of values for lambda
        #epsilon_o = eps_1 + eps_2
        for lambda_o in lambda_o_vec:
     
            epsilon_o = lambda_o * eps_1 + eps_2
            Wls = LwF_based_ADMM_Diff_LS(X_train, Y_train, Wls_prev, lambda_o, epsilon_o, mu)
            # Task prediction on the joint dataset
            acc_train_Wls_LwF_1, acc_test_Wls_LwF_1, nme_test_Wls_LwF_1 = compute_LS_test_accuracy(Wls, X_joint_train, Y_joint_train, X1_test_append, Y1_test_append)   
            acc_train_Wls_LwF_2, acc_test_Wls_LwF_2, nme_test_Wls_LwF_2 = compute_LS_test_accuracy(Wls, X_joint_train, Y_joint_train, X2_test_append, Y2_test_append)
            ...
            test_acc_vec_1.append(acc_test_Wls_LwF_1)
            test_acc_vec_2.append(acc_test_Wls_LwF_2)

        # Plotting the results of the param sweep
        title = "Plotting test accuracy for LwF for LS versus Lambda_o (Diff dataset)"
        plot_acc_vs_hyperparam(np.log10(lambda_o_vec), test_acc_vec, test_acc_vec_1, test_acc_vec_2,title)
        lambda_o_optimal = lambda_o_vec[np.argmax(np.array(test_acc_vec))] # Choosing the mu value that gives maximum accuracy
        
        #title = "Plotting T1 test accuracy for LwF for LS versus Lambda_o (Diff dataset)"
        #plot_acc_vs_hyperparam(np.log10(lambda_o_vec), test_acc_vec_1, title)
        #lambda_o_optimal_1 = lambda_o_vec[np.argmax(np.array(test_acc_vec_1))] # Choosing the mu value that gives maximum accuracy
        
        #title = "Plotting T2 test accuracy for LwF for LS versus Lambda_o (Diff dataset)"
        #plot_acc_vs_hyperparam(np.log10(lambda_o_vec), test_acc_vec_2, title)
        #lambda_o_optimal_2 = lambda_o_vec[np.argmax(np.array(test_acc_vec_2))] # Choosing the mu value that gives maximum accuracy
        ...
        #epsilon_o = eps_1 + eps_2
        # Sweeping over a list of values for lambda
        epsilon_o = lambda_o_optimal * eps_1 + eps_2
        for mu in mu_vec:
        ...
        # Plotting the results of the param sweep
        title = "Plotting test accuracy for LwF for LS versus mu (Diff dataset)"
        plot_acc_vs_hyperparam(np.log10(mu_vec), test_acc_vec, test_acc_vec_1, test_acc_vec_2, title)
        mu_optimal = mu_vec[np.argmax(np.array(test_acc_vec))] # Choosing the mu value that gives maximum accuracy
        
        #title = "Plotting T1 test accuracy for LwF for LS versus mu (Diff dataset)"
        #plot_acc_vs_hyperparam(np.log10(mu_vec), test_acc_vec_1, title)
        #mu_optimal_1 = mu_vec[np.argmax(np.array(test_acc_vec_1))] # Choosing the mu value that gives maximum accuracy
        
        #title = "Plotting T2 test accuracy for LwF for LS versus mu (Diff dataset)"
        #plot_acc_vs_hyperparam(np.log10(mu_vec), test_acc_vec_2, title)
        #mu_optimal_2 = mu_vec[np.argmax(np.array(test_acc_vec_2))] # Choosing the mu value that gives maximum accuracy
        ...
        test_acc_vec_1 = []
        test_acc_vec_2 = []
        eps_vec = np.geomspace(1, 400, 20) 
        
        #epsilon_o = eps_1 + eps_2
        # Sweeping over a list of values for lambda
        epsilon_o = lambda_o_optimal * eps_1 + eps_2
        for epsilon_o in eps_vec:
            
            Wls = LwF_based_ADMM_Diff_LS(X_train, Y_train, Wls_prev, lambda_o_optimal, epsilon_o, mu_optimal)
            # Task prediction on the joint dataset
            acc_train_Wls_LwF_1, acc_test_Wls_LwF_1, nme_test_Wls_LwF_1 = compute_LS_test_accuracy(Wls, X_joint_train, Y_joint_train, X1_test_append, Y1_test_append)   
            acc_train_Wls_LwF_2, acc_test_Wls_LwF_2, nme_test_Wls_LwF_2 = compute_LS_test_accuracy(Wls, X_joint_train, Y_joint_train, X2_test_append, Y2_test_append)
            acc_train_Wls_LwF, acc_test_Wls_LwF, nme_test_Wls_LwF = compute_LS_test_accuracy(Wls, X_joint_train, Y_joint_train, X_joint_test, Y_joint_test)         
        ...
        # Plotting the results of the param sweep
        title = "Plotting joint test accuracy for LwF for LS versus Eps (Diff dataset)"
        plot_acc_vs_hyperparam(np.log10(eps_vec), test_acc_vec, test_acc_vec_1, test_acc_vec_2, title)
        eps_optimal = eps_vec[np.argmax(np.array(test_acc_vec))] # Choosing the mu value that gives maximum accuracy
        
        #title = "Plotting T1 test accuracy for LwF for LS versus Eps (Diff dataset)"
        #plot_acc_vs_hyperparam(np.log10(eps_vec), test_acc_vec_1, title)
        #eps_optimal_1 = eps_vec[np.argmax(np.array(test_acc_vec_1))] # Choosing the mu value that gives maximum accuracy
        
        #title = "Plotting T2 test accuracy for LwF for LS versus Eps (Diff dataset)"
        #plot_acc_vs_hyperparam(np.log10(eps_vec), test_acc_vec_2, title)
        #eps_optimal_2 = eps_vec[np.argmax(np.array(test_acc_vec_2))] # Choosing the mu value that gives maximum accuracy
...
###########################################################################################
def param_tuning_for_O(pln_object, Y_train,  X_test, Y_test, Wls, max_ADMM_iterations, no_layers=None):
    ...
    # pln_object: Objects obtained upto zeroth layer (after Least Squares)
    # no_layers: No. of total layers in the PLN

    # The param is swept over from 10^-14 to 10^14, and the param with the highest test_acc is chosen 
    
    mu_jt_vec = np.geomspace(1e-6, 1e6, 13) 
    test_acc_vec = []
    #pln_array = []
    # Sweeping over a list of values for lambda
    for mu in mu_jt_vec:
        ...
        print("************** Tuning mu={}, ADMM for Layer No:{} **************".format(mu,1))
        pln_object.O_l = compute_ol(pln_object.Y_l, Y_train, mu, max_ADMM_iterations)
        pln_array.append(pln_object)
        
        Q = Y_train.shape[0]
        # Going for all the remaining layers through for loop
        for i in range(1, no_layers):
            
            print("**************  Tuning mu={}, ADMM for Layer No:{} **************".format(mu,i+1))
            X = pln_array[i-1].Y_l # Input is the Output g(WX) for the previous layer
            ...
            W_top = np.dot(np.dot(pln.V_Q,pln_array[i-1].O_l), X)
            ...
            pln.O_l = compute_ol(pln.Y_l, Y_train, mu, max_ADMM_iterations)
            # Add the new layer to the 'Network' list
            pln_array.append(pln)
            # Compute training, test accuracy, NME for new appended networks
            predicted_lbl_test = compute_test_outputs(pln_array, Wls, i+1, X_test)
        
        # Task prediction on the joint dataset
        predicted_lbl_test = compute_test_outputs(pln_array, Wls, no_layers, X_test)
        test_acc_vec.append(compute_accuracy(predicted_lbl_test, Y_test))

        # Pop out the elements of the array again

        for _ in range(no_layers):
            pln_array.pop() # The array should be empty at this point
...
###########################################################################################
def param_tuning_for_O_LwF(pln_object, T2_train, X2_test, T2_test, O1_star, Wls, epsilon_o, mu, eps2=None, eps1=None, no_layers=None):
...
# O1_star : Can be the past O matrix / array of objects containing past O matrix
...
# The param is swept over from 10^-10 to 10^10, and the param with the highest test_acc is chosen (Currently optimisation over only the 1st layer)
    ...
    # tuning lambda
    lambda_o_vec = np.geomspace(1e-10, 1e5, 16) 
    test_acc_vec = []
    test_acc_vec_1 = []
    test_acc_vec_2 = []
    ...
    for lambda_o in lambda_o_vec:
        ...
        pln_array = []
        mu = 1e3
        # Tuning for the first layer
        epsilon_o = lambda_o * eps1 + eps2
        print("**************  Tuning lambda={}, ADMM for Layer No:{} **************".format(lambda_o,1))
        pln_object.O_l = LwF_based_ADMM_Diff_O(pln_object.Y_l, T2_train, O1_star[0].O_l, lambda_o, epsilon_o, mu)
        pln_array.append(pln_object)
        
        # Tuning for the remaining layers
        Q = T2_train.shape[0] + O1_star[0].O_l.shape[0]
        # Going for all the remaining layers through for loop
        for i in range(1, no_layers):
            
            print("**************  Tuning lambda={}, ADMM for Layer No:{} **************".format(lambda_o,i+1))
            X = pln_array[i-1].Y_l # Input is the Output g(WX) for the previous layer
            num_node = 2*Q + 1000 # No. of nodes fixed for every layer
            pln = PLN(Q, X, i, num_node, W_ls=None, R_flag="DiffO", R_old=O1_star[i].R_l) # Creating the PLN Object for the new layer
            # Compute the top part of the Composite Weight Matrix
            W_top = np.dot(np.dot(pln.V_Q,pln_array[i-1].O_l), X)
            # Compute the bottom part of the Composite Weight Matrix
            W_bottom = pln.normalization(np.dot(pln.R_l, X))
            ...
            pln.Y_l = pln.activation_function(pln_Z_l)
            # Compute the output matrix using ADMM for specified no. of iterations
            pln.O_l = LwF_based_ADMM_Diff_O(pln.Y_l, T2_train, O1_star[i].O_l, lambda_o, epsilon_o, mu)
            # Add the new layer to the 'Network' list
            pln_array.append(pln)
            # Compute training, test accuracy, NME for new appended networks
            predicted_lbl_test = compute_test_outputs(pln_array, Wls, i+1, X_joint_test)
        
        # Task prediction on the joint dataset
        predicted_lbl_test = compute_test_outputs(pln_array, Wls, no_layers, X_joint_test)
        test_acc_vec.append(compute_accuracy(predicted_lbl_test, Y_joint_test))

        predicted_lbl_test1 = compute_test_outputs(pln_array, Wls, no_layers, X1_test_append)
        test_acc_vec_1.append(compute_accuracy(predicted_lbl_test1, Y1_test_append))

        predicted_lbl_test2 = compute_test_outputs(pln_array, Wls, no_layers, X2_test_append)
        test_acc_vec_2.append(compute_accuracy(predicted_lbl_test2, Y2_test_append))
    ...
    # Plotting the results of the param sweep
    title = "Plotting test accuracy for LwF for O_L vs lambda_o (Diff dataset)"
    plot_acc_vs_hyperparam(np.log10(lambda_o_vec), test_acc_vec, test_acc_vec_1, test_acc_vec_2,title)
    lambda_o_optimal = lambda_o_vec[np.argmax(np.array(test_acc_vec))] # Choosing the mu value that gives maximum accuracy
    
    #title = "Plotting T1 test accuracy for LwF for O_L vs lambda_o (Diff dataset)"
    #plot_acc_vs_hyperparam(np.log10(lambda_o_vec), test_acc_vec_1, title)
    #lambda_o_optimal1 = lambda_o_vec[np.argmax(np.array(test_acc_vec_1))] # Choosing the mu value that gives maximum accuracy

    #title = "Plotting T2 test accuracy for LwF for O_L vs lambda_o (Diff dataset)"
    #plot_acc_vs_hyperparam(np.log10(lambda_o_vec), test_acc_vec_2, title)
    #lambda_o_optimal2 = lambda_o_vec[np.argmax(np.array(test_acc_vec_2))] # Choosing the mu value that gives maximum accuracy
    ...
    for mu in mu_vec:
        ...
        print("**************  Tuning mu={}, ADMM for Layer No:{} **************".format(mu,1))
        pln_object.O_l = LwF_based_ADMM_Diff_O(pln_object.Y_l, T2_train, O1_star[0].O_l, lambda_o_optimal, epsilon_o, mu)
        pln_array.append(pln_object)
        ...
        for i in range(1, no_layers):
            ...
            pln.Y_l = pln.activation_function(pln_Z_l)
            # Compute the output matrix using ADMM for specified no. of iterations
            pln.O_l = LwF_based_ADMM_Diff_O(pln.Y_l, T2_train, O1_star[i].O_l, lambda_o_optimal, epsilon_o, mu)
            # Add the new layer to the 'Network' list
            pln_array.append(pln)
            # Compute training, test accuracy, NME for new appended networks
            predicted_lbl_test = compute_test_outputs(pln_array, Wls, i+1, X_joint_test)
    ...
    # Plotting the results of the param sweep
    #title = "Plotting test accuracy for LwF for O_L vs mu (Diff dataset)"
    #plot_acc_vs_hyperparam(np.log10(mu_vec), test_acc_vec, test_acc_vec_1, test_acc_vec_2, title)
    mu_optimal = mu_vec[np.argmax(np.array(test_acc_vec))] # Choosing the mu value that gives maximum accuracy
...
#################################################################################################
# Creates two PLN object arrays from the joint array output
#################################################################################################
...
#######################################################################################################
def plot_acc_vs_hyperparam(hyperparam_vec, test_acc_vec, test_acc_vec_1, test_acc_vec_2, title):
...
def PLN_with_ADMM(X_train, Y_train, X_test, Y_test, no_layers, max_iterations, lambda_ls = None, mu = None, LwF_flag = False,
    ...
    if LwF_flag ==  False:
        ...
        ##########################################################################################
        if mu == None:
            mu_optimal = param_tuning_for_O(pln_l1, Y_train, X_test, Y_test, Wls, max_iterations, no_layers=no_layers)
        ...
        if lambda_ls != None:
            ...
            # Tune the lambda for the LS for given dataset
            lambda_ls_optimal, mu_ls_optimal = param_tuning_for_LS(X_train, Y_train, X_test, Y_test, LwF_flag=True, Wls_prev=W_ls_prev, epsilon_o=eps_Wls, mu=None, eps_2=eps_Wls_2, eps_1=eps_Wls_1) 
        ...
        # Create an object of PLN Class
        pln_l1 = PLN(Q, X_train, layer_no, num_node, W_ls = Wls, R_flag="DiffWls", R_old=O_l_prev_array[0].R_l)
        # Compute the top part of the Composite Weight Matrix
        W_top = np.dot(np.dot(pln_l1.V_Q, Wls), X_train)
        # Compute the Bottom part of the Composite Weight Matrix and inner product with input, along with including normalization
        W_bottom = pln_l1.normalization(np.dot(pln_l1.R_l, X_train)) # Normalization performed is for the random matrix
        ...
        ##########################################################################################
        if mu == None:
            lambda_o_optimal, mu_optimal = param_tuning_for_O_LwF(pln_l1, Y_train, X_test, Y_test, O_l_prev_array, Wls, eps_o, mu, eps2=eps_o_2, eps1=eps_o_1, no_layers=3)
        ...
        for i in range(1, no_layers):
            ...
            X = PLN_objects[i-1].Y_l # Input is the Output g(WX) for the previous layer
            num_node = 2*Q + 1000 # No. of nodes fixed for every layer
            pln = PLN(Q, X, i, num_node, W_ls=None, R_flag="DiffO", R_old=O_l_prev_array[i].R_l) # Creating the PLN Object for the new layer
            # Compute the top part of the Composite Weight Matrix
            W_top = np.dot(np.dot(pln.V_Q,PLN_objects[i-1].O_l), X)
            # Compute the bottom part of the Composite Weight Matrix
            W_bottom = pln.normalization(np.dot(pln.R_l, X))
...
def main():
    ...
    dataset_1_name = "ExtendedYaleB" # Specify the Dataset name without extension (implicitly .mat extension is assumed)
    #dataset_2_name = "AR"
    ...
    Y1_train = Y_train[:int(Y_train.shape[0]/2), X1_Y1_train_indices]

    X2_train = X_train[:, X2_Y2_train_indices]
    Y2_train = Y_train[int(Y_train.shape[0]/2):, X2_Y2_train_indices]
    ...
    Y1_test = Y_test[:int(Y_test.shape[0]/2), X1_Y1_test_indices]

    X2_test = X_test[:, X2_Y2_test_indices]
    Y2_test = Y_test[int(Y_test.shape[0]/2):, X2_Y2_test_indices]
    ...
    mu1 = 1e1 # For the given dataset
    lambda_ls1 = 1e5 # Given regularization parameter as used in the paper for the used Dataset
    ...
    lambda_ls1 = 1e4 # Given regularization parameter as used in the paper for the used Dataset
    mu2 = 1e3 # For the given dataset
    lambda_ls2 = 1e4 # Given regularization parameter as used in the paper for the used Dataset
    ##########################################################################################
    ...
    no_layers = 20
    alpha = 2# Has to be at minimum 1
    ###############################################################################################
    ...
    #epsilon_o_Wls = copy.deepcopy(epsilon_o)
